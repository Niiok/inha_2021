// WindowsControl.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_WINDOWSCONTROL, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINDOWSCONTROL));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINDOWSCONTROL));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_WINDOWSCONTROL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_EXIT:
			...
			break;
		case ID_FUNC5:
			DialogBox(hInst, MAKEINTRESOURCE(IDD_CONTROL_DIALOG), hWnd, CONTROL_DIALOG_Proc);
			break;
		case ID_FUNC6:
			if (!IsWindow(hDlg))
			{
				hDlg = CreateDialog(hInst, MAKEINTRESOURCE(IDD_CONTROL_DIALOG), hWnd, CONTROL_DIALOG_Proc);
